...
import AdminExamMarksEntry from './AdminExamMarksEntry';
import MeritListPage from './MeritListPage';
import OverallProgressReport from './OverallProgressReport';
import { ExamAttendanceReport } from './ExamAttendanceReport';

const AdminExams: React.FC = () => {
    const { user, availableBatches } = useAuth();
    
    const [view, setView] = useState<'list' | 'marks' | 'merit' | 'report' | 'attendance'>('list');
    ...
    if (loading) {
      return <div className="p-4">Loading exam data...</div>
    }
    ...
    if (view === 'report' && currentBatchObject) {
        return <OverallProgressReport currentBatch={currentBatchObject} onBack={resetView} />;
    }

    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100">Exam Information for {user?.currentBatch} Batch</h1>
                <div className="flex items-center space-x-2">
                    <button onClick={() => setView('report')} className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 flex items-center space-x-2">
