...
import { Exam, Student, Batch, ExamResult } from '../../types';

interface OverallProgressReportProps {
    currentBatch: Batch;
    onBack: () => void;
}
...
const OverallProgressReport: React.FC<OverallProgressReportProps> = ({ currentBatch, onBack }) => {
        ...
        const fetchConfigData = async () => {
            ...
            try {
                const [exams, students] = await Promise.all([
                    api.getExams(currentBatch.name),
                    api.getStudentsByBatch(currentBatch.name)
                ]);
                setAllExams(exams);
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            const perExamRankMap = new Map<string, Map<string, number>>(); // examId -> (studentId -> rank)
            ...
            // Generate report data with ranks
            const generatedData = selectedStudentIds.map(studentId => {
                ...
                const studentResults = selectedExamIds.map(examId => {
                    ...
                    return {
                        ...
                        examRank: rank || 0, // 0 if not ranked (e.g., absent)
                    };
                });
    ...
    if (loading && configView) return <p>Loading configuration...</p>;
                        ...
                        <div key={student.id} className="report-card bg-white dark:bg-slate-800 rounded-lg shadow-md p-6">
                            ...
                            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    {results.map((r, index) => {
                                        ...
                                        const examDateFormatted = r.examDate ? new Date(r.examDate).toLocaleDateString() : 'N/A';
                                        return (
                                            <tr key={r.examId} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                                ...
                                                <td className="px-4 py-2">{examDateFormatted}</td>
                            ...
                            <div className="mt-8 pt-4 border-t border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400">
                                <div className="flex justify-between items-end">
                                    <div>
                                        <p><strong>Place:</strong> Ahilyanagar</p>
        ...
        <div>
            <div className="no-print">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md">
                    <h1 className="text-2xl font-bold mb-4">Overall Progress Report Generator</h1>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {/* Exam Selection */}
                        <div>
                            <div className="flex justify-between items-center border-b pb-2 mb-2">
                                <h2 className="text-lg font-semibold">1. Select Exams</h2>
                                <label className="text-sm flex items-center"><input type="checkbox" onChange={e => handleSelectAllExams(e.target.checked)} className="mr-2"/> Select All</label>
                            </div>
                        ...
                        {/* Student Selection */}
                         <div>
                            <div className="flex justify-between items-center border-b pb-2 mb-2">
                                ...
                                <label className="text-sm flex items-center"><input type="checkbox" onChange={e => handleSelectAllStudents(e.target.checked)} className="mr-2"/> Select All (Visible)</label>
                            </div>
                             <input
                                type="text"
